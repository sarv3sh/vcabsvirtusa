...
<nav class="navbar navbar-expand-lg navbar-light " style="font-size: x-large;color: white;    background-color: black;">
  ...
  <input type="text" id="myInput" onkeyup="myFunction()" placeholder="Search for locations.." title="Type in your location">
		...
		<tr>
			...
			<th style="margin-left:10px;">BOOK YOUR PASSENGER</th>
		</tr>
		</thead>
		<tr>
		<%
		
		String driver = (String)request.getAttribute("drivername");
		...
		try{
			...
			while(rs.next())
			{
				...
				<td>
				<a href="delete.jsp?u=<%=rs.getInt("id")%>&v=<%=rs.getString("pickuploc")%>&w=<%=rs.getString("droplocation") %>&x=<%=rs.getString("rateinr") %>&y=<%=rs.getString("rateusd") %>&z=<%=rs.getString("distance") %>&a=<%=rs.getString("dat") %>&b=<%=rs.getString("tim") %>" class="btn btn-danger" style="text-align:center;margin-left: 50px;">BOOK</a>
  ...
  for (i = 0; i < tr.length; i++) {
    ...
    if (td) {
      ...
      if (txtValue.toUpperCase().indexOf(filter) > -1) {
		...
		<footer class="footer navbar-static-bottom"  style="background-color: black;margin-top: 500px;">
